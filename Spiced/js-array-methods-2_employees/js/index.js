...
function findByLastName(employees, lastName) {
  ...
  return employees.find(
    (employee) => employee.lastName.toLowerCase() === normalizedLastName
  );
}
// Exercise 2:
...
function findByCitySubString(employees, string) {
  ...
  return employees.find((employee) =>
    employee.city.toLowerCase().includes(normalizedString)
  );
}
...
function findByProfessionSubStringAndAge(employees, string, age) {
  const normalizedStringProfession = string.toLowerCase();
  return employees.find(
    (employee) =>
      employee.profession.toLowerCase().includes(normalizedStringProfession) &&
      employee.age > age
  );
}
